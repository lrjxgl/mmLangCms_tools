...
/*解决import未使用*/
func ForumImgNull(c echo.Context) (err error){
...
/*@@ForumImgIndex@@*/
func ForumImgIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []forumModel.ForumImgModel{}
	...
	var rscount int64;
	db.Model(&forumModel.ForumImgModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []forumModel.ForumImgModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = forumModel.ForumImgList(list)
...
/*@@ForumImgAdd@@*/
func ForumImgAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = forumModel.ForumImgModel{}
	if id != 0 {
...
/*@@ForumImgSave@@*/
func ForumImgSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(forumModel.ForumImgModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ForumImgDelete@@*/
func ForumImgDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(forumModel.ForumImgModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(forumModel.ForumImgModel{}).Where("id=?", id).Update("status", 11)
