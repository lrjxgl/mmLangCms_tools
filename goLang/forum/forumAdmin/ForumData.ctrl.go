...
/*解决import未使用*/
func ForumDataNull(c echo.Context) (err error){
...
/*@@ForumDataIndex@@*/
func ForumDataIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []forumModel.ForumDataModel{}
	...
	var rscount int64;
	db.Model(&forumModel.ForumDataModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []forumModel.ForumDataModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = forumModel.ForumDataList(list)
...
/*@@ForumDataAdd@@*/
func ForumDataAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = forumModel.ForumDataModel{}
	if id != 0 {
...
/*@@ForumDataSave@@*/
func ForumDataSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(forumModel.ForumDataModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ForumDataDelete@@*/
func ForumDataDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(forumModel.ForumDataModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(forumModel.ForumDataModel{}).Where("id=?", id).Update("status", 11)
