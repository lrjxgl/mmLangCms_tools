...
/*解决import未使用*/
func InviteNull(c echo.Context) (err error){
...
/*@@InviteIndex@@*/
func InviteIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.InviteModel{}
	...
	var rscount int64;
	db.Model(&indexModel.InviteModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.InviteModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.InviteList(list)
...
/*@@InviteList@@*/
func InviteList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@InviteShow@@*/
func InviteShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.InviteModel)
...
/*@@InviteMy@@*/
func InviteMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@InviteAdd@@*/
func InviteAdd(c echo.Context) (err error) {
	...
	var data = indexModel.InviteModel{}
	if id != 0 {
		res := db.Where("id=?  ", id).First(&data)
		if res.Error != nil {
			return config.Success(c, 1, "数据不存在")
...
/*@@InviteSave@@*/
func InviteSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.InviteModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@InviteDelete@@*/
func InviteDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.InviteModel{}).Where("id=?", id).Update("status", 11)
