...
/*解决import未使用*/
func TixianNull(c echo.Context) (err error){
...
/*@@TixianIndex@@*/
func TixianIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.TixianModel{}
	...
	var rscount int64;
	db.Model(&indexModel.TixianModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.TixianModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.TixianList(list)
...
/*@@TixianList@@*/
func TixianList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@TixianShow@@*/
func TixianShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.TixianModel)
