...
/*解决import未使用*/
func AttachNull(c echo.Context) (err error){
...
/*@@AttachIndex@@*/
func AttachIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.AttachModel{}
	...
	var rscount int64;
	db.Model(&indexModel.AttachModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.AttachModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.AttachList(list)
...
/*@@AttachList@@*/
func AttachList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@AttachShow@@*/
func AttachShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.AttachModel)
...
/*@@AttachMy@@*/
func AttachMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@AttachAdd@@*/
func AttachAdd(c echo.Context) (err error) {
	...
	var data = indexModel.AttachModel{}
	if id != 0 {
...
/*@@AttachSave@@*/
func AttachSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.AttachModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@AttachStatus@@*/
func AttachStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.AttachModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@AttachDelete@@*/
func AttachDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.AttachModel{}).Where("id=?", id).Update("status", 11)
