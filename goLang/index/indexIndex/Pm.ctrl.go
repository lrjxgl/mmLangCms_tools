...
/*解决import未使用*/
func PmNull(c echo.Context) (err error){
...
/*@@PmIndex@@*/
func PmIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.PmModel{}
	...
	var rscount int64;
	db.Model(&indexModel.PmModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PmModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.PmList(list)
...
/*@@PmList@@*/
func PmList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@PmShow@@*/
func PmShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PmModel)
...
/*@@PmMy@@*/
func PmMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@PmAdd@@*/
func PmAdd(c echo.Context) (err error) {
...
/*@@PmSave@@*/
func PmSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.PmModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PmStatus@@*/
func PmStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.PmModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@PmDelete@@*/
func PmDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.PmModel{}).Where("id=?", id).Update("status", 11)
