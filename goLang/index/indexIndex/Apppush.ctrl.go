...
/*解决import未使用*/
func ApppushNull(c echo.Context) (err error){
...
/*@@ApppushIndex@@*/
func ApppushIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.ApppushModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ApppushModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ApppushModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ApppushList(list)
...
/*@@ApppushList@@*/
func ApppushList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@ApppushShow@@*/
func ApppushShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ApppushModel)
...
/*@@ApppushMy@@*/
func ApppushMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@ApppushAdd@@*/
func ApppushAdd(c echo.Context) (err error) {
	...
	var data = indexModel.ApppushModel{}
	if id != 0 {
		res := db.Where("id=?  ", id).First(&data)
		if res.Error != nil {
			return config.Success(c, 1, "数据不存在")
...
/*@@ApppushSave@@*/
func ApppushSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.ApppushModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ApppushDelete@@*/
func ApppushDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.ApppushModel{}).Where("id=?", id).Update("status", 11)
