...
/*解决import未使用*/
func SmsLogNull(c echo.Context) (err error){
...
/*@@SmsLogIndex@@*/
func SmsLogIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.SmsLogModel{}
	...
	var rscount int64;
	db.Model(&indexModel.SmsLogModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.SmsLogModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.SmsLogList(list)
...
/*@@SmsLogList@@*/
func SmsLogList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@SmsLogShow@@*/
func SmsLogShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.SmsLogModel)
...
/*@@SmsLogMy@@*/
func SmsLogMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@SmsLogAdd@@*/
func SmsLogAdd(c echo.Context) (err error) {
	...
	var data = indexModel.SmsLogModel{}
	if id != 0 {
...
/*@@SmsLogSave@@*/
func SmsLogSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.SmsLogModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@SmsLogStatus@@*/
func SmsLogStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.SmsLogModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@SmsLogDelete@@*/
func SmsLogDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.SmsLogModel{}).Where("id=?", id).Update("status", 11)
