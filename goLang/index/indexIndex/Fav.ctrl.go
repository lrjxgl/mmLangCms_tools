...
/*解决import未使用*/
func FavNull(c echo.Context) (err error){
...
/*@@FavIndex@@*/
func FavIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.FavModel{}
	...
	var rscount int64;
	db.Model(&indexModel.FavModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.FavModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.FavList(list)
...
/*@@FavList@@*/
func FavList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FavShow@@*/
func FavShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.FavModel)
...
/*@@FavMy@@*/
func FavMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FavAdd@@*/
func FavAdd(c echo.Context) (err error) {
...
/*@@FavSave@@*/
func FavSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.FavModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@FavDelete@@*/
func FavDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.FavModel{}).Where("id=?", id).Update("status", 11)
