...
/*解决import未使用*/
func RefundNull(c echo.Context) (err error){
...
/*@@RefundIndex@@*/
func RefundIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.RefundModel{}
	...
	var rscount int64;
	db.Model(&indexModel.RefundModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.RefundModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.RefundList(list)
...
/*@@RefundList@@*/
func RefundList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@RefundShow@@*/
func RefundShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.RefundModel)
...
/*@@RefundMy@@*/
func RefundMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@RefundAdd@@*/
func RefundAdd(c echo.Context) (err error) {
	...
	var data = indexModel.RefundModel{}
	if id != 0 {
...
/*@@RefundSave@@*/
func RefundSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.RefundModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@RefundStatus@@*/
func RefundStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.RefundModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@RefundDelete@@*/
func RefundDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.RefundModel{}).Where("id=?", id).Update("status", 11)
