...
/*解决import未使用*/
func UserAddressNull(c echo.Context) (err error){
...
/*@@UserAddressIndex@@*/
func UserAddressIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.UserAddressModel{}
	...
	var rscount int64;
	db.Model(&indexModel.UserAddressModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.UserAddressModel{}
	}
	//输出浏览器
...
/*@@UserAddressList@@*/
func UserAddressList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@UserAddressShow@@*/
func UserAddressShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.UserAddressModel)
...
/*@@UserAddressMy@@*/
func UserAddressMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@UserAddressAdd@@*/
func UserAddressAdd(c echo.Context) (err error) {
	...
	var data = indexModel.UserAddressModel{}
	if id != 0 {
...
/*@@UserAddressSave@@*/
func UserAddressSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.UserAddressModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@UserAddressDelete@@*/
func UserAddressDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.UserAddressModel{}).Where("id=?", id).Update("status", 11)
