...
/*解决import未使用*/
func FollowNull(c echo.Context) (err error){
...
/*@@FollowIndex@@*/
func FollowIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.FollowModel{}
	...
	var rscount int64;
	db.Model(&indexModel.FollowModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.FollowModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.FollowList(list)
...
/*@@FollowList@@*/
func FollowList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FollowShow@@*/
func FollowShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.FollowModel)
...
/*@@FollowMy@@*/
func FollowMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FollowAdd@@*/
func FollowAdd(c echo.Context) (err error) {
	...
	var data = indexModel.FollowModel{}
	if id != 0 {
...
/*@@FollowSave@@*/
func FollowSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.FollowModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@FollowStatus@@*/
func FollowStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.FollowModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@FollowDelete@@*/
func FollowDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.FollowModel{}).Where("id=?", id).Update("status", 11)
