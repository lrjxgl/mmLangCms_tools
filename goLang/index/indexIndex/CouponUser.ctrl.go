...
/*解决import未使用*/
func CouponUserNull(c echo.Context) (err error){
...
/*@@CouponUserIndex@@*/
func CouponUserIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.CouponUserModel{}
	...
	var rscount int64;
	db.Model(&indexModel.CouponUserModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.CouponUserModel{}
	}
	//输出浏览器
...
/*@@CouponUserList@@*/
func CouponUserList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@CouponUserShow@@*/
func CouponUserShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.CouponUserModel)
...
/*@@CouponUserMy@@*/
func CouponUserMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@CouponUserAdd@@*/
func CouponUserAdd(c echo.Context) (err error) {
	...
	var data = indexModel.CouponUserModel{}
	if id != 0 {
...
/*@@CouponUserSave@@*/
func CouponUserSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.CouponUserModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@CouponUserDelete@@*/
func CouponUserDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.CouponUserModel{}).Where("id=?", id).Update("status", 11)
