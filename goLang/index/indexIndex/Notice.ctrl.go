...
/*解决import未使用*/
func NoticeNull(c echo.Context) (err error){
...
/*@@NoticeIndex@@*/
func NoticeIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.NoticeModel{}
	...
	var rscount int64;
	db.Model(&indexModel.NoticeModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.NoticeModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.NoticeList(list)
...
/*@@NoticeList@@*/
func NoticeList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@NoticeShow@@*/
func NoticeShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.NoticeModel)
...
/*@@NoticeMy@@*/
func NoticeMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@NoticeAdd@@*/
func NoticeAdd(c echo.Context) (err error) {
	...
	var data = indexModel.NoticeModel{}
	if id != 0 {
...
/*@@NoticeSave@@*/
func NoticeSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.NoticeModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@NoticeStatus@@*/
func NoticeStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.NoticeModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@NoticeDelete@@*/
func NoticeDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.NoticeModel{}).Where("id=?", id).Update("status", 11)
