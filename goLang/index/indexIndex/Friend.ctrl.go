...
/*解决import未使用*/
func FriendNull(c echo.Context) (err error){
...
/*@@FriendIndex@@*/
func FriendIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.FriendModel{}
	...
	var rscount int64;
	db.Model(&indexModel.FriendModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.FriendModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.FriendList(list)
...
/*@@FriendList@@*/
func FriendList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FriendShow@@*/
func FriendShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.FriendModel)
...
/*@@FriendMy@@*/
func FriendMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@FriendAdd@@*/
func FriendAdd(c echo.Context) (err error) {
	...
	var data = indexModel.FriendModel{}
	if id != 0 {
		res := db.Where("id=?  ", id).First(&data)
		if res.Error != nil {
			return config.Success(c, 1, "数据不存在")
...
/*@@FriendSave@@*/
func FriendSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.FriendModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@FriendDelete@@*/
func FriendDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.FriendModel{}).Where("id=?", id).Update("status", 11)
