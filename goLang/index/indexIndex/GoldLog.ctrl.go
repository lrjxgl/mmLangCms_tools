...
/*解决import未使用*/
func GoldLogNull(c echo.Context) (err error){
...
/*@@GoldLogIndex@@*/
func GoldLogIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.GoldLogModel{}
	...
	var rscount int64;
	db.Model(&indexModel.GoldLogModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.GoldLogModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.GoldLogList(list)
...
/*@@GoldLogList@@*/
func GoldLogList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@GoldLogShow@@*/
func GoldLogShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.GoldLogModel)
...
/*@@GoldLogMy@@*/
func GoldLogMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@GoldLogAdd@@*/
func GoldLogAdd(c echo.Context) (err error) {
	...
	var data = indexModel.GoldLogModel{}
	if id != 0 {
		res := db.Where("id=?  ", id).First(&data)
		if res.Error != nil {
			return config.Success(c, 1, "数据不存在")
...
/*@@GoldLogSave@@*/
func GoldLogSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.GoldLogModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@GoldLogDelete@@*/
func GoldLogDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.GoldLogModel{}).Where("id=?", id).Update("status", 11)
