...
/*解决import未使用*/
func InviteAccountNull(c echo.Context) (err error){
...
/*@@InviteAccountIndex@@*/
func InviteAccountIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.InviteAccountModel{}
	...
	var rscount int64;
	db.Model(&indexModel.InviteAccountModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.InviteAccountModel{}
	}
	//输出浏览器
...
/*@@InviteAccountList@@*/
func InviteAccountList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@InviteAccountShow@@*/
func InviteAccountShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.InviteAccountModel)
...
/*@@InviteAccountMy@@*/
func InviteAccountMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@InviteAccountAdd@@*/
func InviteAccountAdd(c echo.Context) (err error) {
	...
	var data = indexModel.InviteAccountModel{}
	if id != 0 {
		res := db.Where("id=?  ", id).First(&data)
		if res.Error != nil {
			return config.Success(c, 1, "数据不存在")
...
/*@@InviteAccountSave@@*/
func InviteAccountSave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.InviteAccountModel{}).Where("id=?", id).Updates(postData)
	}
...
func InviteAccountDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.InviteAccountModel{}).Where("id=?", id).Update("status", 11)
