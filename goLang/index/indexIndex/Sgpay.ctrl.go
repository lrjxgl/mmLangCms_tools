...
/*解决import未使用*/
func SgpayNull(c echo.Context) (err error){
...
/*@@SgpayIndex@@*/
func SgpayIndex(c echo.Context) (err error) {
	fmt.Print("forumIndex")
	
	var db = config.Db
	var list = []indexModel.SgpayModel{}
	...
	var rscount int64;
	db.Model(&indexModel.SgpayModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.SgpayModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.SgpayList(list)
...
/*@@SgpayList@@*/
func SgpayList(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@SgpayShow@@*/
func SgpayShow(c echo.Context) (err error) {
	
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.SgpayModel)
...
/*@@SgpayMy@@*/
func SgpayMy(c echo.Context) (err error) {
	fmt.Print("forumIndex")
...
/*@@SgpayAdd@@*/
func SgpayAdd(c echo.Context) (err error) {
	...
	var data = indexModel.SgpayModel{}
	if id != 0 {
...
/*@@SgpaySave@@*/
func SgpaySave(c echo.Context) (err error) {
	...
	if id != 0 {
		...
		db.Model(indexModel.SgpayModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@SgpayStatus@@*/
func SgpayStatus(c echo.Context) (err error) {
	...
	db.Model(indexModel.SgpayModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@SgpayDelete@@*/
func SgpayDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.SgpayModel{}).Where("id=?", id).Update("status", 11)
