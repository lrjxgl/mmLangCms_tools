...
/*解决import未使用*/
func ConfigNull(c echo.Context) (err error){
...
/*@@ConfigIndex@@*/
func ConfigIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ConfigModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ConfigModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ConfigModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ConfigList(list)
...
/*@@ConfigAdd@@*/
func ConfigAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ConfigModel{}
	if id != 0 {
...
/*@@ConfigSave@@*/
func ConfigSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ConfigModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ConfigDelete@@*/
func ConfigDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ConfigModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.ConfigModel{}).Where("id=?", id).Update("status", 11)
