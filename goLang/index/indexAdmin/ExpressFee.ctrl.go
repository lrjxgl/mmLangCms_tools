...
/*解决import未使用*/
func ExpressFeeNull(c echo.Context) (err error){
...
/*@@ExpressFeeIndex@@*/
func ExpressFeeIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ExpressFeeModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ExpressFeeModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ExpressFeeModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ExpressFeeList(list)
...
/*@@ExpressFeeAdd@@*/
func ExpressFeeAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ExpressFeeModel{}
	if id != 0 {
...
/*@@ExpressFeeSave@@*/
func ExpressFeeSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ExpressFeeModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ExpressFeeDelete@@*/
func ExpressFeeDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ExpressFeeModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.ExpressFeeModel{}).Where("id=?", id).Update("status", 11)
