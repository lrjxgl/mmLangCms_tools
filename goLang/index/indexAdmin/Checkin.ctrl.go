...
/*解决import未使用*/
func CheckinNull(c echo.Context) (err error){
...
/*@@CheckinIndex@@*/
func CheckinIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.CheckinModel{}
	...
	var rscount int64;
	db.Model(&indexModel.CheckinModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.CheckinModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.CheckinList(list)
...
/*@@CheckinAdd@@*/
func CheckinAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.CheckinModel{}
	if id != 0 {
...
/*@@CheckinSave@@*/
func CheckinSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.CheckinModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@CheckinDelete@@*/
func CheckinDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.CheckinModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.CheckinModel{}).Where("id=?", id).Update("status", 11)
