...
/*解决import未使用*/
func CouponNull(c echo.Context) (err error){
...
/*@@CouponIndex@@*/
func CouponIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.CouponModel{}
	...
	var rscount int64;
	db.Model(&indexModel.CouponModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.CouponModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.CouponList(list)
...
/*@@CouponAdd@@*/
func CouponAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.CouponModel{}
	if id != 0 {
...
/*@@CouponSave@@*/
func CouponSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.CouponModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@CouponStatus@@*/
func CouponStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.CouponModel)
	...
	db.Model(indexModel.CouponModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
func CouponDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.CouponModel{}).Where("id=?", id).Update("status", 11)
