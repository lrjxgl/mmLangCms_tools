...
/*解决import未使用*/
func ModuleNull(c echo.Context) (err error){
...
/*@@ModuleIndex@@*/
func ModuleIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ModuleModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ModuleModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ModuleModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ModuleList(list)
...
/*@@ModuleAdd@@*/
func ModuleAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ModuleModel{}
	if id != 0 {
...
/*@@ModuleSave@@*/
func ModuleSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ModuleModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ModuleDelete@@*/
func ModuleDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ModuleModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.ModuleModel{}).Where("id=?", id).Update("status", 11)
