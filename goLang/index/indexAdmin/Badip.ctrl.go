...
/*解决import未使用*/
func BadipNull(c echo.Context) (err error){
...
/*@@BadipIndex@@*/
func BadipIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.BadipModel{}
	...
	var rscount int64;
	db.Model(&indexModel.BadipModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.BadipModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.BadipList(list)
...
/*@@BadipAdd@@*/
func BadipAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.BadipModel{}
	if id != 0 {
...
/*@@BadipSave@@*/
func BadipSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.BadipModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@BadipDelete@@*/
func BadipDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.BadipModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.BadipModel{}).Where("id=?", id).Update("status", 11)
