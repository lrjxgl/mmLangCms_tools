...
/*解决import未使用*/
func ArticleDataNull(c echo.Context) (err error){
...
/*@@ArticleDataIndex@@*/
func ArticleDataIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ArticleDataModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ArticleDataModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ArticleDataModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ArticleDataList(list)
...
/*@@ArticleDataAdd@@*/
func ArticleDataAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ArticleDataModel{}
	if id != 0 {
...
/*@@ArticleDataSave@@*/
func ArticleDataSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ArticleDataModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ArticleDataDelete@@*/
func ArticleDataDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ArticleDataModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.ArticleDataModel{}).Where("id=?", id).Update("status", 11)
