...
/*解决import未使用*/
func AdminNull(c echo.Context) (err error){
...
/*@@AdminIndex@@*/
func AdminIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.AdminModel{}
	...
	var rscount int64;
	db.Model(&indexModel.AdminModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.AdminModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.AdminList(list)
...
/*@@AdminAdd@@*/
func AdminAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.AdminModel{}
	if id != 0 {
...
/*@@AdminSave@@*/
func AdminSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.AdminModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@AdminDelete@@*/
func AdminDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.AdminModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.AdminModel{}).Where("id=?", id).Update("status", 11)
