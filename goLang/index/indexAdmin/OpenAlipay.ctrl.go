...
/*解决import未使用*/
func OpenAlipayNull(c echo.Context) (err error){
...
/*@@OpenAlipayIndex@@*/
func OpenAlipayIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.OpenAlipayModel{}
	...
	var rscount int64;
	db.Model(&indexModel.OpenAlipayModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.OpenAlipayModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.OpenAlipayList(list)
...
/*@@OpenAlipayAdd@@*/
func OpenAlipayAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.OpenAlipayModel{}
	if id != 0 {
...
/*@@OpenAlipaySave@@*/
func OpenAlipaySave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.OpenAlipayModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@OpenAlipayStatus@@*/
func OpenAlipayStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.OpenAlipayModel)
	...
	db.Model(indexModel.OpenAlipayModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@OpenAlipayDelete@@*/
func OpenAlipayDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.OpenAlipayModel{}).Where("id=?", id).Update("status", 11)
