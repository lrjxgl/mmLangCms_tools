...
/*解决import未使用*/
func TixianLogNull(c echo.Context) (err error){
...
/*@@TixianLogIndex@@*/
func TixianLogIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.TixianLogModel{}
	...
	var rscount int64;
	db.Model(&indexModel.TixianLogModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.TixianLogModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.TixianLogList(list)
...
/*@@TixianLogAdd@@*/
func TixianLogAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.TixianLogModel{}
	if id != 0 {
...
/*@@TixianLogSave@@*/
func TixianLogSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.TixianLogModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@TixianLogDelete@@*/
func TixianLogDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.TixianLogModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.TixianLogModel{}).Where("id=?", id).Update("status", 11)
