...
/*解决import未使用*/
func PermissionNull(c echo.Context) (err error){
...
/*@@PermissionIndex@@*/
func PermissionIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.PermissionModel{}
	...
	var rscount int64;
	db.Model(&indexModel.PermissionModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PermissionModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.PermissionList(list)
...
/*@@PermissionAdd@@*/
func PermissionAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.PermissionModel{}
	if id != 0 {
...
/*@@PermissionSave@@*/
func PermissionSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.PermissionModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PermissionDelete@@*/
func PermissionDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PermissionModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.PermissionModel{}).Where("id=?", id).Update("status", 11)
