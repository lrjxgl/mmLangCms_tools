...
/*解决import未使用*/
func PvDayNull(c echo.Context) (err error){
...
/*@@PvDayIndex@@*/
func PvDayIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.PvDayModel{}
	...
	var rscount int64;
	db.Model(&indexModel.PvDayModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PvDayModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.PvDayList(list)
...
/*@@PvDayAdd@@*/
func PvDayAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.PvDayModel{}
	if id != 0 {
...
/*@@PvDaySave@@*/
func PvDaySave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.PvDayModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PvDayDelete@@*/
func PvDayDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PvDayModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.PvDayModel{}).Where("id=?", id).Update("status", 11)
