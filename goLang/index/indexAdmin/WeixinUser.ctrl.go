...
/*解决import未使用*/
func WeixinUserNull(c echo.Context) (err error){
...
/*@@WeixinUserIndex@@*/
func WeixinUserIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.WeixinUserModel{}
	...
	var rscount int64;
	db.Model(&indexModel.WeixinUserModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.WeixinUserModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.WeixinUserList(list)
...
/*@@WeixinUserAdd@@*/
func WeixinUserAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.WeixinUserModel{}
	if id != 0 {
...
/*@@WeixinUserSave@@*/
func WeixinUserSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.WeixinUserModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@WeixinUserStatus@@*/
func WeixinUserStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.WeixinUserModel)
	...
	db.Model(indexModel.WeixinUserModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@WeixinUserDelete@@*/
func WeixinUserDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.WeixinUserModel{}).Where("id=?", id).Update("status", 11)
