...
/*解决import未使用*/
func PagetplNull(c echo.Context) (err error){
...
/*@@PagetplIndex@@*/
func PagetplIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.PagetplModel{}
	...
	var rscount int64;
	db.Model(&indexModel.PagetplModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PagetplModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.PagetplList(list)
...
/*@@PagetplAdd@@*/
func PagetplAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.PagetplModel{}
	if id != 0 {
...
/*@@PagetplSave@@*/
func PagetplSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.PagetplModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PagetplDelete@@*/
func PagetplDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PagetplModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.PagetplModel{}).Where("id=?", id).Update("status", 11)
