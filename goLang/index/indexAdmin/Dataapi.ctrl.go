...
/*解决import未使用*/
func DataapiNull(c echo.Context) (err error){
...
/*@@DataapiIndex@@*/
func DataapiIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.DataapiModel{}
	...
	var rscount int64;
	db.Model(&indexModel.DataapiModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.DataapiModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.DataapiList(list)
...
/*@@DataapiAdd@@*/
func DataapiAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.DataapiModel{}
	if id != 0 {
...
/*@@DataapiSave@@*/
func DataapiSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.DataapiModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@DataapiStatus@@*/
func DataapiStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.DataapiModel)
	...
	db.Model(indexModel.DataapiModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@DataapiDelete@@*/
func DataapiDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.DataapiModel{}).Where("id=?", id).Update("status", 11)
