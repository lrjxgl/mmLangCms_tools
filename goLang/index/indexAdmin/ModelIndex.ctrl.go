...
/*解决import未使用*/
func ModelIndexNull(c echo.Context) (err error){
...
/*@@ModelIndexIndex@@*/
func ModelIndexIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ModelIndexModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ModelIndexModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ModelIndexModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ModelIndexList(list)
...
/*@@ModelIndexAdd@@*/
func ModelIndexAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ModelIndexModel{}
	if id != 0 {
...
/*@@ModelIndexSave@@*/
func ModelIndexSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ModelIndexModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ModelIndexDelete@@*/
func ModelIndexDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ModelIndexModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.ModelIndexModel{}).Where("id=?", id).Update("status", 11)
