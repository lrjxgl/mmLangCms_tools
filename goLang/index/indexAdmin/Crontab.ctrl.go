...
/*解决import未使用*/
func CrontabNull(c echo.Context) (err error){
...
/*@@CrontabIndex@@*/
func CrontabIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.CrontabModel{}
	...
	var rscount int64;
	db.Model(&indexModel.CrontabModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.CrontabModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.CrontabList(list)
...
/*@@CrontabAdd@@*/
func CrontabAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.CrontabModel{}
	if id != 0 {
...
/*@@CrontabSave@@*/
func CrontabSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.CrontabModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@CrontabStatus@@*/
func CrontabStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.CrontabModel)
	...
	db.Model(indexModel.CrontabModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@CrontabDelete@@*/
func CrontabDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.CrontabModel{}).Where("id=?", id).Update("status", 11)
