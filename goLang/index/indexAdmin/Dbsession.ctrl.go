...
/*解决import未使用*/
func DbsessionNull(c echo.Context) (err error){
...
/*@@DbsessionIndex@@*/
func DbsessionIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.DbsessionModel{}
	...
	var rscount int64;
	db.Model(&indexModel.DbsessionModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.DbsessionModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.DbsessionList(list)
...
/*@@DbsessionAdd@@*/
func DbsessionAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.DbsessionModel{}
	if id != 0 {
...
/*@@DbsessionSave@@*/
func DbsessionSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.DbsessionModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@DbsessionDelete@@*/
func DbsessionDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.DbsessionModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.DbsessionModel{}).Where("id=?", id).Update("status", 11)
