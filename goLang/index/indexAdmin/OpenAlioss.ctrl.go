...
/*解决import未使用*/
func OpenAliossNull(c echo.Context) (err error){
...
/*@@OpenAliossIndex@@*/
func OpenAliossIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.OpenAliossModel{}
	...
	var rscount int64;
	db.Model(&indexModel.OpenAliossModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.OpenAliossModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.OpenAliossList(list)
...
/*@@OpenAliossAdd@@*/
func OpenAliossAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.OpenAliossModel{}
	if id != 0 {
...
/*@@OpenAliossSave@@*/
func OpenAliossSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.OpenAliossModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@OpenAliossStatus@@*/
func OpenAliossStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.OpenAliossModel)
	...
	db.Model(indexModel.OpenAliossModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@OpenAliossDelete@@*/
func OpenAliossDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.OpenAliossModel{}).Where("id=?", id).Update("status", 11)
