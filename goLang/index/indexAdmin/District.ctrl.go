...
func DistrictIndex(c echo.Context) (err error) {
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.DistrictList(list)
...
/*@@DistrictAdd@@*/
func DistrictAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.DistrictModel{}
	if id != 0 {
...
/*@@DistrictSave@@*/
func DistrictSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.DistrictModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@DistrictDelete@@*/
func DistrictDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.DistrictModel{}).Where("id=?", id).Update("status", 11)
