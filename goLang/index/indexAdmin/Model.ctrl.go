...
/*解决import未使用*/
func ModelNull(c echo.Context) (err error){
...
/*@@ModelIndex@@*/
func ModelIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ModelModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ModelModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ModelModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.ModelList(list)
...
/*@@ModelAdd@@*/
func ModelAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ModelModel{}
	if id != 0 {
...
/*@@ModelSave@@*/
func ModelSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ModelModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@ModelStatus@@*/
func ModelStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ModelModel)
	...
	db.Model(indexModel.ModelModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
func ModelDelete(c echo.Context) (err error) {
	...
	db.Model(indexModel.ModelModel{}).Where("id=?", id).Update("status", 11)
