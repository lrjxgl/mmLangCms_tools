...
/*解决import未使用*/
func QueueNull(c echo.Context) (err error){
...
/*@@QueueIndex@@*/
func QueueIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.QueueModel{}
	...
	var rscount int64;
	db.Model(&indexModel.QueueModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.QueueModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.QueueList(list)
...
/*@@QueueAdd@@*/
func QueueAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.QueueModel{}
	if id != 0 {
...
/*@@QueueSave@@*/
func QueueSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.QueueModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@QueueDelete@@*/
func QueueDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.QueueModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.QueueModel{}).Where("id=?", id).Update("status", 11)
