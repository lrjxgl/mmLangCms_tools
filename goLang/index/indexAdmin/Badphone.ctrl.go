...
/*解决import未使用*/
func BadphoneNull(c echo.Context) (err error){
...
/*@@BadphoneIndex@@*/
func BadphoneIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.BadphoneModel{}
	...
	var rscount int64;
	db.Model(&indexModel.BadphoneModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.BadphoneModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.BadphoneList(list)
...
/*@@BadphoneAdd@@*/
func BadphoneAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.BadphoneModel{}
	if id != 0 {
...
/*@@BadphoneSave@@*/
func BadphoneSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.BadphoneModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@BadphoneDelete@@*/
func BadphoneDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.BadphoneModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.BadphoneModel{}).Where("id=?", id).Update("status", 11)
