...
/*解决import未使用*/
func OpenQqNull(c echo.Context) (err error){
...
/*@@OpenQqIndex@@*/
func OpenQqIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.OpenQqModel{}
	...
	var rscount int64;
	db.Model(&indexModel.OpenQqModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.OpenQqModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.OpenQqList(list)
...
/*@@OpenQqAdd@@*/
func OpenQqAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.OpenQqModel{}
	if id != 0 {
...
/*@@OpenQqSave@@*/
func OpenQqSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.OpenQqModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@OpenQqStatus@@*/
func OpenQqStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.OpenQqModel)
	...
	db.Model(indexModel.OpenQqModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@OpenQqDelete@@*/
func OpenQqDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.OpenQqModel{}).Where("id=?", id).Update("status", 11)
