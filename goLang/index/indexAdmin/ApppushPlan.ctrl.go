...
/*解决import未使用*/
func ApppushPlanNull(c echo.Context) (err error){
...
/*@@ApppushPlanIndex@@*/
func ApppushPlanIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.ApppushPlanModel{}
	...
	var rscount int64;
	db.Model(&indexModel.ApppushPlanModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.ApppushPlanModel{}
	}
	//输出浏览器
...
/*@@ApppushPlanAdd@@*/
func ApppushPlanAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.ApppushPlanModel{}
	if id != 0 {
...
/*@@ApppushPlanSave@@*/
func ApppushPlanSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.ApppushPlanModel{}).Where("id=?", id).Updates(postData)
	}
...
func ApppushPlanStatus(c echo.Context) (err error) {
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.ApppushPlanModel)
...
/*@@ApppushPlanDelete@@*/
func ApppushPlanDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.ApppushPlanModel{}).Where("id=?", id).Update("status", 11)
