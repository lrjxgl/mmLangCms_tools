...
/*解决import未使用*/
func OpenWxappNull(c echo.Context) (err error){
...
/*@@OpenWxappIndex@@*/
func OpenWxappIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.OpenWxappModel{}
	...
	var rscount int64;
	db.Model(&indexModel.OpenWxappModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.OpenWxappModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.OpenWxappList(list)
...
/*@@OpenWxappAdd@@*/
func OpenWxappAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.OpenWxappModel{}
	if id != 0 {
...
/*@@OpenWxappSave@@*/
func OpenWxappSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.OpenWxappModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@OpenWxappStatus@@*/
func OpenWxappStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.OpenWxappModel)
	...
	db.Model(indexModel.OpenWxappModel{}).Where("id=?", id).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@OpenWxappDelete@@*/
func OpenWxappDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.OpenWxappModel{}).Where("id=?", id).Update("status", 11)
