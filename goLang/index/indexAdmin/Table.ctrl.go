...
/*@@TableAdd@@*/
func TableAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}

	tableid, err := strconv.Atoi(c.QueryParam("tableid"))
	var db = config.Db

	var data = indexModel.TableModel{}
	if tableid != 0 {
		res := db.Where("tableid=?  AND status<4  ", tableid).First(&data)
		if res.Error != nil {
	...
	reJson["message"] = "success"
	reJson["data"] = data
	reJson["tableid"] = tableid
	return c.JSON(http.StatusOK, reJson)
}

/*@@TableSave@@*/
func TableSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}

	tableid, err := strconv.Atoi(c.FormValue("tableid"))
	var db = config.Db
	var data = indexModel.TableModel{}
	if tableid != 0 {
		res := db.Where("tableid=?  AND status<4  ", tableid).First(&data)
	...
	if tableid != 0 {
		...
		db.Model(indexModel.TableModel{}).Where("tableid=?", tableid).Updates(postData)
	}
...
/*@@TableStatus@@*/
func TableStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	res := db.Where("tableid=?", tableid).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	...
	db.Model(indexModel.TableModel{}).Where("tableid=?", tableid).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@TableDelete@@*/
func TableDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.TableModel{}).Where("tableid=?", tableid).Update("status", 11)
