...
/*解决import未使用*/
func AdminGroupNull(c echo.Context) (err error){
...
/*@@AdminGroupIndex@@*/
func AdminGroupIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.AdminGroupModel{}
	...
	var rscount int64;
	db.Model(&indexModel.AdminGroupModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.AdminGroupModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.AdminGroupList(list)
...
/*@@AdminGroupAdd@@*/
func AdminGroupAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.AdminGroupModel{}
	if id != 0 {
...
/*@@AdminGroupSave@@*/
func AdminGroupSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.AdminGroupModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@AdminGroupDelete@@*/
func AdminGroupDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.AdminGroupModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.AdminGroupModel{}).Where("id=?", id).Update("status", 11)
