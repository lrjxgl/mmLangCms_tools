...
/*解决import未使用*/
func DaysnNull(c echo.Context) (err error){
...
/*@@DaysnIndex@@*/
func DaysnIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.DaysnModel{}
	...
	var rscount int64;
	db.Model(&indexModel.DaysnModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.DaysnModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.DaysnList(list)
...
/*@@DaysnAdd@@*/
func DaysnAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.DaysnModel{}
	if id != 0 {
...
/*@@DaysnSave@@*/
func DaysnSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.DaysnModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@DaysnDelete@@*/
func DaysnDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.DaysnModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.DaysnModel{}).Where("id=?", id).Update("status", 11)
