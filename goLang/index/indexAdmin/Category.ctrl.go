...
/*解决import未使用*/
func CategoryNull(c echo.Context) (err error){
...
/*@@CategoryIndex@@*/
func CategoryIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.CategoryModel{}
	...
	var rscount int64;
	db.Model(&indexModel.CategoryModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.CategoryModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.CategoryList(list)
...
/*@@CategoryAdd@@*/
func CategoryAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.CategoryModel{}
	if catid != 0 {
...
/*@@CategorySave@@*/
func CategorySave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if catid != 0 {
		...
		db.Model(indexModel.CategoryModel{}).Where("catid=?", catid).Updates(postData)
	}
...
/*@@CategoryStatus@@*/
func CategoryStatus(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var db = config.Db
	data := new(indexModel.CategoryModel)
	...
	db.Model(indexModel.CategoryModel{}).Where("catid=?", catid).Update("status", status)
	reJson := make(map[string]interface{})
	reJson["error"] = 0
	reJson["message"] = "success"
...
/*@@CategoryDelete@@*/
func CategoryDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.CategoryModel{}).Where("catid=?", catid).Update("status", 11)
