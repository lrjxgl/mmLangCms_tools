...
/*解决import未使用*/
func DbcacheNull(c echo.Context) (err error){
...
/*@@DbcacheIndex@@*/
func DbcacheIndex(c echo.Context) (err error) {
	...
	var db = config.Db
	var list = []indexModel.DbcacheModel{}
	...
	var rscount int64;
	db.Model(&indexModel.DbcacheModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.DbcacheModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.DbcacheList(list)
...
/*@@DbcacheAdd@@*/
func DbcacheAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.DbcacheModel{}
	if id != 0 {
...
/*@@DbcacheSave@@*/
func DbcacheSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.DbcacheModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@DbcacheDelete@@*/
func DbcacheDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.DbcacheModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.DbcacheModel{}).Where("id=?", id).Update("status", 11)
