...
/*解决import未使用*/
func PvUvNull(c echo.Context) (err error){
...
/*@@PvUvIndex@@*/
func PvUvIndex(c echo.Context) (err error) {
	...
	var rscount int64;
	db.Model(&indexModel.PvUvModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PvUvModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.PvUvList(list)
...
/*@@PvUvAdd@@*/
func PvUvAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
...
/*@@PvUvSave@@*/
func PvUvSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.PvUvModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PvUvDelete@@*/
func PvUvDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PvUvModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.PvUvModel{}).Where("id=?", id).Update("status", 11)
