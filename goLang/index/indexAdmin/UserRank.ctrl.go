...
/*@@UserRankAdd@@*/
func UserRankAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	var data = indexModel.UserRankModel{}
	if id != 0 {
...
/*@@UserRankSave@@*/
func UserRankSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.UserRankModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@UserRankDelete@@*/
func UserRankDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	db.Model(indexModel.UserRankModel{}).Where("id=?", id).Update("status", 11)
