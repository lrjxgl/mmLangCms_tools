...
/*解决import未使用*/
func SeoNull(c echo.Context) (err error){
...
/*@@SeoIndex@@*/
func SeoIndex(c echo.Context) (err error) {
	...
	var rscount int64;
	db.Model(&indexModel.SeoModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.SeoModel{}
	}
	//输出浏览器
	...
	reJson["error"] = 0
	reJson["message"] = "success"
	reJson["list"] = indexModel.SeoList(list)
...
/*@@SeoAdd@@*/
func SeoAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
...
/*@@SeoSave@@*/
func SeoSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.SeoModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@SeoDelete@@*/
func SeoDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.SeoModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.SeoModel{}).Where("id=?", id).Update("status", 11)
