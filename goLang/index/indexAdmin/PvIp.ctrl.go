...
/*解决import未使用*/
func PvIpNull(c echo.Context) (err error){
...
/*@@PvIpIndex@@*/
func PvIpIndex(c echo.Context) (err error) {
	...
	var rscount int64;
	db.Model(&indexModel.PvIpModel{}).Where(where).Count(&rscount);
	//获取列表
	res := db.Where(where).Limit(limit).Offset(start).Find(&list)
	if res.Error != nil {
		list = []indexModel.PvIpModel{}
	}
	//输出浏览器
...
/*@@PvIpAdd@@*/
func PvIpAdd(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
...
/*@@PvIpSave@@*/
func PvIpSave(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	if id != 0 {
		...
		db.Model(indexModel.PvIpModel{}).Where("id=?", id).Updates(postData)
	}
...
/*@@PvIpDelete@@*/
func PvIpDelete(c echo.Context) (err error) {
	adminid := access.AdminCheckAccess(c)
	if adminid == 0 {
		return config.Success(c, 1000, "暂无权限")
	}
	...
	id := c.QueryParam("id")
	var db = config.Db
	data := new(indexModel.PvIpModel)
	res := db.Where("id=?", id).First(&data)
	if res.Error != nil {
		return config.Success(c, 1, "数据不存在")
	}
	
	db.Model(indexModel.PvIpModel{}).Where("id=?", id).Update("status", 11)
