...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
						<td>tagid：</td>		
						<td><input class="input-text" type="text" name="tagid"   v-model="data.tagid" ></td>
						</tr>
  <tr>
						<td>objectid：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/forum_tags_index/add",
					data: {
						'.id.': this.'.id.'
					},
...
orderindex: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/forum_tags_index/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
