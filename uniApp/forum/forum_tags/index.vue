		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="tagid" v-model="id" />
					<text>状态：</text>
...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   ...
   <td>{{item.gnum}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.tagid)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/forum_tags/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/forum_tags/status",
					data:{
						tagid:item.tagid
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/forum_tags/recommend",
					data:{
						tagid:item.tagid
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/forum_tags/delete",
							data:{
								tagid:item.tagid
							},
							success:function(res){
								...
								for(var i in that.list){
									if(that.list[i].tagid!=item.tagid){
