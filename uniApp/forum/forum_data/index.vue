		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="did" v-model="id" />
					<text>状态：</text>
...
<td>操作</td></tr>
   ...
   <td>{{item.id}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.did)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/forum_data/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/forum_data/status",
					data:{
						did:item.did
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/forum_data/recommend",
					data:{
						did:item.did
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/forum_data/delete",
							data:{
								did:item.did
							},
							success:function(res){
