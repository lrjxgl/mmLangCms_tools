...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>fuserid：</td>		
						<td><input class="input-text" type="text" name="fuserid"   v-model="data.fuserid" ></td>
						</tr>
  <tr>
						<td>createtime：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/forum_feeds/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/forum_feeds/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
