		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="gid" v-model="id" />
					<text>状态：</text>
...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   <td>{{item.topic_num}}</td>
   <td>{{item.comment_num}}</td>
   <td>{{item.view_num}}</td>
<td><div @click="toggleRecommend(item)" :class="item.isrecommend==1?'yes':'no'"></div></td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.gid)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/forum_group/status",
					data:{
						gid:item.gid
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/forum_group/delete",
							data:{
								gid:item.gid
							},
							success:function(res){
