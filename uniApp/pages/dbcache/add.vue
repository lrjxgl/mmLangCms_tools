...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>v：</td>		
						<td><input class="input-text" type="text" name="v"   v-model="data.v" ></td>
						</tr>
  <tr>
						<td>expire：</td>		
						<td><input class="input-text" type="text" name="expire"   v-model="data.expire" ></td>
						</tr>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/dbcache/add",
					data: {
						'.id.': this.'.id.'
					},
...
expire: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/dbcache/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
