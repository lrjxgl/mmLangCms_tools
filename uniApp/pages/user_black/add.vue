...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>被拉黑用户：</td>		
						<td><input class="input-text" type="text" name="buserid"   v-model="data.buserid" ></td>
						</tr>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/user_black/add",
					data: {
						'.id.': this.'.id.'
					},
...
buserid: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/user_black/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
