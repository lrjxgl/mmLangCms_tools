...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>区域：</td>		
						<td><input class="input-text" type="text" name="areaid"   v-model="data.areaid" ></td>
						</tr>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/express_fee_city/add",
					data: {
						'.id.': this.'.id.'
					},
...
areaid: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/express_fee_city/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
