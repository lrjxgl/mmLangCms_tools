...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   <td>{{item.appcert_path}}</td>
   <td>{{item.alicert_path}}</td>
   <td>{{item.rootcert_path}}</td>
<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/open_alipay_mini/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/open_alipay_mini/status",
					data:{
						id:item.id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/open_alipay_mini/recommend",
					data:{
						id:item.id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/open_alipay_mini/delete",
							data:{
								id:item.id
							},
							success:function(res){
