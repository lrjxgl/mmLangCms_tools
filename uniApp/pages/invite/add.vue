...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>被邀请人：</td>		
						<td><input class="input-text" type="text" name="in_userid"   v-model="data.in_userid" ></td>
						</tr>
  <tr>
						<td>是否奖赏：</td>		
  ...
  <tr>
						<td>状态：</td>		
						<td><input class="input-text" type="text" name="bstatus"   v-model="data.bstatus" ></td>
						</tr>
  <tr>
						<td>createtime：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/invite/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/invite/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
