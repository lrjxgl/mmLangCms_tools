		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="uid" v-model="id" />
					<text>状态：</text>
...
<td>操作</td></tr>
   ...
   <td>{{item.uid}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.uid)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/user_expand/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/user_expand/status",
					data:{
						uid:item.uid
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/user_expand/recommend",
					data:{
						uid:item.uid
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/user_expand/delete",
							data:{
								uid:item.uid
							},
							success:function(res){
