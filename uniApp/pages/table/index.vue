		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="tableid" v-model="id" />
					<text>状态：</text>
...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   ...
   <td>{{item.addTpl}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.tableid)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/table/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/table/status",
					data:{
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/table/recommend",
					data:{
						tableid:item.tableid
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/table/delete",
							data:{
								tableid:item.tableid
							},
							success:function(res){
								...
								for(var i in that.list){
									if(that.list[i].tableid!=item.tableid){
			...
			goAdd:function(tableid){
				uni.navigateTo({
					url:"add?tableid="+tableid
				})
			},
			goShow:function(id){
				uni.navigateTo({
