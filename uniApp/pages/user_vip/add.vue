...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>首次购买：</td>		
						<td><input class="input-text" type="text" name="firsttime"   v-model="data.firsttime" ></td>
						</tr>
  <tr>
						<td>结束时间：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/user_vip/add",
					data: {
						'.id.': this.'.id.'
					},
...
status: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/user_vip/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
