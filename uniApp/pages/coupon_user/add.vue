...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
						<td>优惠券：</td>		
						<td><input class="input-text" type="text" name="coupon_id"   v-model="data.coupon_id" ></td>
						</tr>
  <tr>
						<td>userid：</td>		
  ...
  <tr>
						<td>上次通知：</td>		
						<td><input class="input-text" type="text" name="notice_time"   v-model="data.notice_time" ></td>
						</tr>
  <tr>
						<td>createtime：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/coupon_user/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/coupon_user/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
