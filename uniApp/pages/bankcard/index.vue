...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   <td>{{item.yhk_name}}</td>
   <td>{{item.yhk_haoma}}</td>
   <td>{{item.yhk_huming}}</td>
   <td>{{item.telephone}}</td>
   <td>{{item.yhk_address}}</td>
   <td>{{item.paytype}}</td>
<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/bankcard/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/bankcard/status",
					data:{
						id:item.id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/bankcard/recommend",
					data:{
						id:item.id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/bankcard/delete",
							data:{
								id:item.id
							},
							success:function(res){
