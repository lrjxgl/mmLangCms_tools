...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>类型：</td>		
						<td><input class="input-text" type="text" name="apptype"   v-model="data.apptype" ></td>
						</tr>
  <tr>
						<td>创建时间：</td>		
  ...
  <tr>
						<td>ENDPOINT：</td>		
						<td><input class="input-text" type="text" name="endpoint"   v-model="data.endpoint" ></td>
						</tr>
  <tr>
						<td>TEST_BUCKET：</td>		
						<td><input class="input-text" type="text" name="bucket"   v-model="data.bucket" ></td>
						</tr>
  <tr>
						<td>状态：</td>		
						<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/open_alioss/add",
					data: {
						'.id.': this.'.id.'
					},
...
status: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/open_alioss/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
