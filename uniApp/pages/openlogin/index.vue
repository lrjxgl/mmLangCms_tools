...
<td>操作</td></tr>
   ...
   <td>{{item.nickname}}</td>
<td><image :src="item.user_head" mode="widthFix" class="wh-60" ></image></td>
   <td>{{item.gender}}</td>
   <td>{{item.unionid}}</td>
<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/openlogin/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/openlogin/status",
					data:{
						id:item.id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/openlogin/recommend",
					data:{
						id:item.id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/openlogin/delete",
							data:{
								id:item.id
							},
							success:function(res){
