...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>touserid：</td>		
						<td><input class="input-text" type="text" name="touserid"   v-model="data.touserid" ></td>
						</tr>
  <tr>
						<td>description：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/friend_apply/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/friend_apply/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
