...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>cuserid：</td>		
						<td><input class="input-text" type="text" name="cuserid"   v-model="data.cuserid" ></td>
						</tr>
  <tr>
						<td>money：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/invite_account_log/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/invite_account_log/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
