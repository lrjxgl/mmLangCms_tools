...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						...
						<td><input class="input-text" type="text" name="alipay_public_key"   v-model="data.alipay_public_key" ></td>
						</tr>
  <tr>
						<td>notify_url：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/open_toutiao/add",
					data: {
						'.id.': this.'.id.'
					},
...
openlogin: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/open_toutiao/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
