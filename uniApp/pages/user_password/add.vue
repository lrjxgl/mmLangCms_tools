...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>支付密码：</td>		
						<td><input class="input-text" type="text" name="paypwd"   v-model="data.paypwd" ></td>
						</tr>
  <tr>
						<td>密码串：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/user_password/add",
					data: {
						'.id.': this.'.id.'
					},
...
salt: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/user_password/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
