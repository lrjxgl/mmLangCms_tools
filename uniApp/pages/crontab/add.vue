...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>执行时间：</td>		
						<td><input class="input-text" type="text" name="crontime"   v-model="data.crontime" ></td>
						</tr>
  <tr>
						<td>状态：</td>		
						<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/crontab/add",
					data: {
						'.id.': this.'.id.'
					},
...
status: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/crontab/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
