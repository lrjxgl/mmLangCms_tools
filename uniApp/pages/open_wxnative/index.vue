...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   <td>{{item.sslcert_path}}</td>
   <td>{{item.sslkey_path}}</td>
   <td>{{item.mchid}}</td>
<td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/open_wxnative/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/open_wxnative/status",
					data:{
						id:item.id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/open_wxnative/recommend",
					data:{
						id:item.id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/open_wxnative/delete",
							data:{
								id:item.id
							},
							success:function(res){
