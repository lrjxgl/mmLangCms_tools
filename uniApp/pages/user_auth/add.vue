...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>简介：</td>		
						<td><input class="input-text" type="text" name="sky_info"   v-model="data.sky_info" ></td>
						</tr>
  <tr>
						<td>createtime：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/user_auth/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/user_auth/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
