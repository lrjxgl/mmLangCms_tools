		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="siteid" v-model="id" />
					<text>状态：</text>
...
<td>操作</td></tr>
   ...
   <td>{{item.domain}}</td>
   <td>{{item.is_open}}</td>
   <td>{{item.close_why}}</td>
<td><image :src="item.logo" mode="widthFix" class="wh-60" ></image></td>
   <td>{{item.icp}}</td>
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   <td>{{item.template}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.siteid)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/site/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/site/status",
					data:{
						siteid:item.siteid
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/site/recommend",
					data:{
						siteid:item.siteid
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/site/delete",
							data:{
								siteid:item.siteid
							},
							success:function(res){
								...
								for(var i in that.list){
									if(that.list[i].siteid!=item.siteid){
