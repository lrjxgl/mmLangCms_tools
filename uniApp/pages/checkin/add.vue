...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>day：</td>		
						<td><input class="input-text" type="text" name="day"   v-model="data.day" ></td>
						</tr>
  <tr>
						<td>mood：</td>		
						<td><input class="input-text" type="text" name="mood"   v-model="data.mood" ></td>
						</tr>
  <tr>
						<td>content：</td>		
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/checkin/add",
					data: {
						'.id.': this.'.id.'
					},
...
createtime: "",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/checkin/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
