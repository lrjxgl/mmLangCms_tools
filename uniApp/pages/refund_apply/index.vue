...
<td>操作</td></tr>
   ...
   <td>{{item.money}}</td>
   <td>{{item.recharge_orderno}}</td>
   <td>{{item.recharge_pay_orderno}}</td>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/refund_apply/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/refund_apply/status",
					data:{
						id:item.id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/refund_apply/recommend",
					data:{
						id:item.id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/refund_apply/delete",
							data:{
								id:item.id
							},
							success:function(res){
