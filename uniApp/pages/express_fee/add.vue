...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>首重：</td>		
						<td><input class="input-text" type="text" name="fmoney"   v-model="data.fmoney" ></td>
						</tr>
  <tr>
						<td>加重：</td>		
						<td><input class="input-text" type="text" name="amoney"   v-model="data.amoney" ></td>
						</tr>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/express_fee/add",
					data: {
						'.id.': this.'.id.'
					},
...
amoney: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/express_fee/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
