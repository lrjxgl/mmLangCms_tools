...
<input type="text" name="id" class="none" v-model="data.id" >
 <table class="table-add">  <tr>
  ...
  <tr>
						<td>isread：</td>		
						<td><input class="input-text" type="text" name="isread"   v-model="data.isread" ></td>
						</tr>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/kefu_msgindex/add",
					data: {
						'.id.': this.'.id.'
					},
...
isread: "0",
			...
			submit: function(e) {
				var that = this;
				that.app.post({
					url: that.app.apiHost + "/admin/kefu_msgindex/save",
					data: e.detail.value,
					success: function(res) {
						uni.showToast({
							title: res.message
