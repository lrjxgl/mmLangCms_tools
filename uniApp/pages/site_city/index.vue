		...
		<div class="search-form">
			<form @submit="search">
				<div class="flex flex-ai-center">
					...
					<input class="w100 mgr-5 input-text" type="text" name="sc_id" v-model="id" />
					<text>状态：</text>
...
<td><div @click="toggleStatus(item)" :class="item.status==1?'yes':'no'"></div></td>
   ...
   <td>{{item.siteid}}</td>
<td>
	<div class="btn-small mgr-5" @click="goAdd(item.sc_id)">编辑</div>
	...
	export default {
		...
		methods: {
			...
			getPage: function() {
				var that = this;
				that.app.get({
					url: that.app.apiHost + "/admin/site_city/index",
			...
			toggleStatus:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/site_city/status",
					data:{
						sc_id:item.sc_id
					},
					success:function(res){
			...
			toggleRecommend:function(item){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/admin/site_city/recommend",
					data:{
						sc_id:item.sc_id
					},
					success:function(res){
			...
			del:function(item){
				...
				uni.showModal({
					...
					success:function(res){
						...
						that.app.get({
							url:that.app.apiHost+"/admin/site_city/delete",
							data:{
								sc_id:item.sc_id
							},
							success:function(res){
								...
								for(var i in that.list){
									if(that.list[i].sc_id!=item.sc_id){
